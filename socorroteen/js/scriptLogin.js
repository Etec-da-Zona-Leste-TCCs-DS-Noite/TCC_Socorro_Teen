...
const signInWithProvider = async (provider, providerName) => {
  try {
    let userCredential;
    try {
      // Tenta popup primeiro (funciona em browsers normais)
      userCredential = await signInWithPopup(auth, provider);
    } catch (popupError) {
      // Se popup falhar (bloqueado), usa redirect (funciona em VMs)
      if (popupError.code === "auth/popup-blocked" || popupError.code === "auth/popup-closed-by-user") {
        console.warn("Popup bloqueado, tentando com redirect...");
        await signInWithRedirect(auth, provider);
        return; // Redirect vai recarregar a página
      }
      throw popupError;
    }
...
// Verifica se há resultado de redirect ao carregar a página
getRedirectResult(auth)
  .then((userCredential) => {
    if (userCredential) {
      console.log("Login por redirect bem-sucedido:", userCredential.user);
      setCurrentUser(userCredential.user.uid);
